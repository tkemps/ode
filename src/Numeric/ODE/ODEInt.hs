...
import Data.Array.Repa (Array(..), D, U, Z(..), (:.)(..), (+^), Source, DIM1,
...
adaptiveStepSizeControl ::
  ...
  ⇒ Array r1 DIM1 Double -- ^ starting point y ∈ R^n
  → (Double, Double) -- ^ starting and end point x0, x1 ∈ R, x1 > x0
  → (∀ r . (Source r Double) ⇒
      Double → Array r DIM1 Double → Array r2 DIM1 Double)
  → Double
  → Double
  → Double
  → s
  → [(Double, Array D DIM1 Double, Double, Double)]
...
dumb ∷ ∀ r1 r2 s . (Source r1 Double, Source r2 Double, Stepper s) ⇒
         Array r1 DIM1 Double
       → (Double, Double)
       → (∀ r . (Source r Double) ⇒ Double → Array r DIM1 Double → Array r2 DIM1 Double)
       → Int
       → Double
       → s
       → [(Double, Array D DIM1 Double)]
dumb y0 (x0, x1) f nStep eps st = rkIter nStep x0 (delay y0)
  where
    shY0@(Z:.n) = extent y0
    h = (x1-x0)/(fromIntegral nStep)
    tiny ∷ Array U DIM1 Double
    tiny = fromListUnboxed shY0 (replicate n 1.0e-30)
    yscal = let dydx = f x0 y0
            in (R.map abs y0) +^ h*.(R.map abs dydx) +^ tiny
    ...
    rkStep x v =
      let dv = f x v
          (x', v', _, _, _, _) = step st v dv x h eps yscal f
...
dumb1 y0 (x0, x1) f nStep eps st = rkIter nStep x0 y0
  where
    ...
    rkStep x v =
      let dv = f x v
          (x', v', _, _, _, _) = step1 st v dv x h eps yscal f
      in if x==x' then throw (StepSizeUnderflow x h) else (x', v')
